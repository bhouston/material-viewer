...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      const sampleIdsFilter = process.env.VIEWER_SAMPLE_IDS?.split(',')
        .map((entry) => entry.trim())
        .filter(Boolean);
      const filteredSamples = sampleIdsFilter
        ? samples.filter((sample) => sampleIdsFilter.includes(sample.id))
        : samples;
      ...
      const targetSamples =
        sampleLimit !== undefined && Number.isFinite(sampleLimit)
          ? filteredSamples.slice(0, sampleLimit)
          : filteredSamples;
